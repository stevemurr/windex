"""The crawl recipe: parse, validate, normalize, round-trip.

A recipe is the whole API contract and the whole unit of reproducibility — every
...
Validation here is a SECURITY BOUNDARY, not a convenience: recipes arrive over the
...
class Recipe:
    ...
    def to_dict(self) -> dict:
        ...
        is what makes a past run's frozen recipe re-runnable verbatim."""
        return {
...
def _clamp_float(value, default: float, lo: float, label: str) -> float:
    ...
    return max(f, lo)  # a recipe may ask to be SLOWER, never faster than the floor
...
def parse(body: dict, settings: Settings) -> Recipe:
    ...
    if prefix is None:
        ...
        # the recipe still means whole-host.
