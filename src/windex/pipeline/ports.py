...
class ExtractedDoc:
    ...
    loader. A Pipeline cannot name an id outside its Source namespace, so it cannot
...
KINDS: dict[str, Kind] = {k.name: k for k in (
    ...
    Kind("receive", "DocumentBatch", "ExtractedDoc", False, "Receive",
...
PORT_TYPES: dict[str, dict] = {
    "DocumentBatch": {"title": "Pushed document batch",
                      "fields": ["documents", "partition", "mode"]},
