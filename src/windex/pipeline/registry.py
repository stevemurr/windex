"""The Module catalog exposed to Pipeline validation and execution.

A Pipeline is inert data. It names Modules; it never carries code, and there is no
import-by-string, no entry-point scan and no plugin directory anywhere in this
package. Local code is admitted only through the separately authenticated,
approved, immutable sandbox lifecycle.
...
`allowed_hosts` is declared by the Module and a Pipeline may only narrow it. Only
...
from windex.pipeline.contracts import REGISTRY_CONTRACT
    ...
    Module(
        ...
        fields=(
            _p("host_interval", "float", lo=1.0, hi=300, default=2.0, unit="s",
               ...
               clamp_note="Raised to the operator's floor if lower. A Pipeline may "
    ...
    Module(
        ...
        fields=(
            ...
            _p("allowed_hosts", "csv", required=True, label="Allowed hosts",
               help="A Pipeline may only narrow what the Module permits."),
...
_CUSTOM_DIGESTS: dict[str, str] = {}
_CUSTOM_EXECUTORS: set[str] = set()
_ROLE_BY_KIND = {
    "discover": ("ingress.pull", "state.read"),
    "receive": ("ingress.push",),
    "collect": ("state.write",),
    "load": ("document.identity", "document.provenance", "document.staging"),
}

for _m in (_DISCOVER + _FETCH + _CATALOG + _EXTRACT + _TRANSFORM + _SINKS):
    _lane, _pre = _PLACEMENT.get(_m.name, ("io", ()))
    MODULES[_m.name] = dataclasses.replace(
        _m,
        lane=_lane,
        preconditions=_pre,
...
def register_custom(descriptor: dict) -> Module:
    ...
    if name in MODULES and name not in _CUSTOM_EXECUTORS:
        raise ValueError(f"custom Module collides with built-in {name!r}")
    accepted = set(inspect.signature(Param).parameters)
    fields = tuple(
        Param(**{key: value for key, value in raw.items() if key in accepted})
        for raw in (descriptor.get("parameter_schema") or [])
    )
    port_spec = descriptor.get("port_spec") or {}
    module = Module(
        name=name,
        kind=str(descriptor["kind"]),
        title=str(descriptor.get("title") or name),
        summary=str(descriptor.get("description") or "Approved local Module"),
        fields=fields,
        capabilities=tuple(descriptor.get("requested_capabilities") or ()),
        allowed_hosts=tuple(descriptor.get("allowed_hosts") or ()),
        lane="cpu_heavy",
        version=str(descriptor["version"]),
        stability="experimental",
        thread_safe=False,
        built_from="local module_admin",
        contract_roles=tuple(port_spec.get("contract_roles") or ()),
    )
    MODULES[name] = module
    _CUSTOM_DIGESTS[name] = str(descriptor["source_digest"])
    _CUSTOM_EXECUTORS.add(name)
    return module


def load_custom(conn) -> int:
    with conn.cursor() as cur:
        cur.execute(
            """SELECT DISTINCT ON (d.name)
                      d.name, d.title, d.description, v.version, v.kind,
                      v.port_spec, v.parameter_schema, v.requested_capabilities,
                      v.allowed_hosts, v.source_digest
                 FROM module_versions v
                 JOIN module_definitions d ON d.id = v.module_id
                WHERE v.approval_state = 'available'
                ORDER BY d.name, v.version DESC""")
        rows = cur.fetchall()
    active: set[str] = set()
    keys = (
        "name", "title", "description", "version", "kind", "port_spec",
        "parameter_schema", "requested_capabilities", "allowed_hosts",
        "source_digest",
    )
    for row in rows:
        descriptor = dict(zip(keys, row))
        register_custom(descriptor)
        active.add(descriptor["name"])
    for name in _CUSTOM_EXECUTORS - active:
        MODULES.pop(name, None)
        _CUSTOM_DIGESTS.pop(name, None)
    _CUSTOM_EXECUTORS.intersection_update(active)
    return len(active)
...
def implemented(name: str) -> bool:
    ...
    return name in runners.RUNNERS or name in _CUSTOM_EXECUTORS


def implementation_digest(name: str) -> str:
    """Stable build digest for the exact in-tree implementation."""
    from windex.pipeline import runners

    module = get(name)
    if name in _CUSTOM_DIGESTS:
        return _CUSTOM_DIGESTS[name]
...
def module_lock(name: str) -> dict[str, str]:
    module = get(name)
    if module is None:
        raise KeyError(name)
    return {
...
def registry_digest() -> str:
    payload = [
        {
            **module.describe(),
            "implemented": implemented(module.name),
            "implementation_digest": implementation_digest(module.name),
        }
...
def describe() -> dict:
    ...
    return {
        ...
        "ports": [
            {"id": name, **descriptor}
            for name, descriptor in ports.PORT_TYPES.items()
        ],
        ...
        "modules": [
            {
                **m.describe(),
                "fields": [field.describe() for field in m.fields],
                "implemented": implemented(m.name),
                "implementation_digest": implementation_digest(m.name),
            }
