"""Durable serialization for the four values that may cross a Pipeline Edge.
...
from windex.pipeline.ports import (
    ExtractedDoc,
    PartitionRecord,
    PartitionRef,
    RawBlob,
...
def encode(value: WireValue) -> dict[str, Any]:
    ...
    raise TypeError(f"cannot encode Pipeline wire value {type(value).__name__}")
...
def decode(raw: Any) -> WireValue:
    ...
    raise ValueError(f"unknown Pipeline wire type {kind!r}")
