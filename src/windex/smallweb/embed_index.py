...
Mirrors the wiki/arxiv embed step: dense vectors from the user's Embedder,
sparse BM25 from fastembed, stable uuid5 point ids, per-batch Postgres commits,
the dashboard pause check, and the runtime throughput profile. Source 'smallweb'
lands in the "smallweb" collection behind the smallweb_current alias. The
embedded text is title + post body; the payload ``outlet`` is the feed host.
"""
...
def _pending_refs(conn: psycopg.Connection, limit: int) -> dict[str, list[str]]:
    """text_ref → doc ids, for the oldest pending smallweb docs."""
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM (
                ...
                WHERE source = 'smallweb' AND status = 'deduped'
...
def embed_pending(conn: psycopg.Connection, settings: Settings, limit: int = 100_000) -> int:
    ...
    collection = qidx.ensure_collection(client, "smallweb", settings.embed_model, settings.embed_dim)
